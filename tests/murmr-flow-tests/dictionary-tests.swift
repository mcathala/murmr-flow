...
struct DictionaryExpanderTests {
    ...
    private func entry(
        ...
        DictionaryEntry(trigger: trigger, replacement: replacement, scope: scope)
    }
    ...
    func basic() {
        let result = DictionaryExpander.expand(
            ...
            using: [entry("my email address", "mc@kovalee.app")]
        )

        #expect(result.text == "send it to [[MF0]] please")
        #expect(result.resolved == "send it to mc@kovalee.app please")
    ...
    func normalisation() {
        ...
        #expect(result.resolved == "mc@kovalee.app, then.")
    }

    @Test("a curly apostrophe matches a straight one")
    func apostrophes() {
        let result = DictionaryExpander.expand(
            "that\u{2019}s maxime\u{2019}s line",
            using: [entry("that's maxime's", "MC:")]
        )

        #expect(result.resolved == "MC: line")
    }
    ...
    @Test("an entry with no trigger is a hint, never a match")
    func hintsAreNotMatched() {
        let hint = DictionaryEntry(replacement: "Kovalee")
        let result = DictionaryExpander.expand("kovalee is the company", using: [hint])

        #expect(result.isEmpty)
        #expect(DictionaryExpander.hints(from: [hint]) == ["Kovalee"])
    }
    ...
    func restore() {
        ...
        #expect(expansion.restore(into: "Send it to [[MF0]].") == "Send it to mc@kovalee.app.")
    }
    ...
    func droppedMarker() {
        ...
        #expect(expansion.resolved == "send it to mc@kovalee.app")
    }
...
struct DictionaryStoreTests {
    ...
    @Test("the old custom words become hints scoped to both jobs")
    func seedsFromLegacyWords() {
        let defaults = scratch("seed")
        defaults.set(["Kovalee", "Murmr"], forKey: "cleanup.customWords")

        let store = DictionaryStore(defaults: defaults)

        #expect(store.entries.count == 2)
        #expect(store.entries.allSatisfy { !$0.isReplacement })
        #expect(store.entries.allSatisfy { $0.scope == .both })
        #expect(store.entries.map(\.replacement) == ["Kovalee", "Murmr"])
    }

    @Test("the words it seeded from are left on disk, not consumed")
    func doesNotDeleteLegacyKey() {
        let defaults = scratch("keep")
        defaults.set(["Kovalee"], forKey: "cleanup.customWords")

        _ = DictionaryStore(defaults: defaults)

        #expect(defaults.stringArray(forKey: "cleanup.customWords") == ["Kovalee"])
    }
    ...
    func storedListWins() {
        ...
        first.add(trigger: "my email", replacement: "mc@kovalee.app", scope: .dictation)
    ...
    func scopedReads() {
        ...
        store.add(trigger: "a", replacement: "1", scope: .dictation)
        store.add(trigger: "b", replacement: "2", scope: .notetaker)
        store.add(trigger: "c", replacement: "3", scope: .both)
    ...
    func skipsUnusable() {
        ...
        store.add(trigger: "a", replacement: "")
